...
async function loadAPIs() {
    try {
        ...
        const apiList = settings.api; // Pastikan path ini sesuai dengan settings.json

        const apiContainer = document.getElementById('apiLinks');
        apiContainer.innerHTML = '';

        apiList.forEach(api => {
            let apiItem = document.createElement('div');
            apiItem.className = 'api-item';
            apiItem.innerHTML = `<a href="${api.url}" target="_blank">${api.name}</a>`;
